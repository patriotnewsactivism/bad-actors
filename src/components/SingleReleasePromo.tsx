...
const SingleReleasePromo = ({
  ...
  const officialVideo = OFFICIAL_VIDEO_BY_TRACK[trackSlug];
    ...
    <section id="single-release" className="relative w-full bg-gradient-to-b from-zinc-950 via-black to-zinc-950 border-y-4 border-crime-yellow py-12 sm:py-16 overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10 max-w-4xl">
        ...
        <p className="text-police-red font-bold uppercase tracking-widest text-center text-sm mb-8">
          Don Matthews · Outlawed Productions
          {officialVideo ? ` · Released ${officialVideo.releaseDate}` : ""}
        </p>
          ...
          <div className="mb-10">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-zinc-300 text-sm sm:text-base leading-relaxed">
                Watch the official music video for <span className="font-semibold text-white">{title}</span> in full.
              </p>
              <a
                href={officialVideo.watchUrl}
                target="_blank"
                rel="noreferrer"
                className="inline-flex shrink-0 items-center justify-center gap-2 border border-police-red/70 px-4 py-2.5 text-sm font-bold uppercase tracking-wide text-white hover:bg-police-red/15 transition-colors"
              >
                Watch fullscreen <ExternalLink className="w-4 h-4" />
              </a>
            </div>
          </div>
        )}
